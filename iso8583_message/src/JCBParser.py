...
from ..ISO8583Message.ISO8583Parser import ISO8583Parser
from utilities.utilities import Logger
import sys
from io import StringIO

class LoggerStream:
    def __init__(self, is_logger):
    ...
    def flush(self):
        if self.is_logger:
            log_message = self.buffer.getvalue().strip()
        if log_message:
...
class JCBParser(ISO8583Parser):
    ...
    def custom_field_decoder(self,msg,msgType:dict[str,CustomField])->list:
        content = []
        if msg is None:
            return content
        length = len(msg)
    ...
    def iso_decode(self, message) -> dict:
        ...
        while lenOfMsg > 0:
            ...
            lenOfMsg = len(message)
            if dict_has_data(parsed_message):
                msgNum += 1
    ...
    def iso_dump(self,iso_dict:dict[int,dict],is_logger:bool=False):
        if is_logger:
            stream = LoggerStream(is_logger)
        else:
            stream = sys.stdout
        ...
        for num, parsed_data in iso_dict.items():
            ...
            for key, isoData in parsed_data.items():
                self.print_iso_field(isoData,stream, 120)
